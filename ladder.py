...
# define mocked data
X_test = np.random.randn(100, 28*28)
...
train_iter = tf.train.GradientDescentOptimizer(learning_rate=0.01).minimize(C_result)
...
with tf.Session() as sess:
    ...
    for e in range(EPOCHS):
        ...
        graph_feed = {x: X_train, y_gt: Y_train}

        _, c_uv, c_sv = sess.run([train_iter, C_uv_sum, C_sv], feed_dict=graph_feed)

        print('\rEpoch {}/{}: C_sv={:.3f} C_uv={:.3f}'.format(e+1,EPOCHS, c_uv, c_sv))